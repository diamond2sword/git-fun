...
# shellcheck disable=SC2034,SC2317,SC2207,SC2046,SC2005,SC2002,SC2086,SC2115,SC2164 
...
declare_git_commands () {
	...
	make_git_bash () {
		...
		repo_name="$(basename $(git config remote.origin.url) | cut -d. -f1)"
		goto "$repo_path"
		...
		cp "$REPO_PATH/$THIS_FILE_NAME" "$update_file"
		...
		[ -f .gitignore ] && {
			sed -i "/MAKE_GIT_BASH/d" ".gitignore"
		}
		echo "git.bash # MAKE_GIT_BASH" >> ".gitignore"
	}
